...
FROM
    {{ source('recommender_graph_db_dev', 'c1_dev_app7020_content') }}
{% else %}
SELECT
...
FROM
    {{ source('recommender_graph_db_dev', 'c2_dev_app7020_content') }}
{% else %}
SELECT
    NULL AS item_id, NULL AS creation_date, NULL AS content_type, NULL AS recency, NULL AS customer_name
WHERE
...
FROM
    {{ source('recommender_graph_db_dev', 'c3_dev_app7020_content') }}
{% else %}
SELECT
    NULL AS item_id, NULL AS creation_date, NULL AS content_type, NULL AS recency, NULL AS customer_name
WHERE
