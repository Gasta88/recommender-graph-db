...
FROM
    {{ source('recommender_graph_db_dev', 'c1_dev_learning_course') }}
{% else %}
SELECT
...
FROM
    {{ source('recommender_graph_db_dev', 'c2_dev_learning_course') }}
{% else %}
SELECT
    NULL AS item_id, NULL AS "name", NULL AS category_id, NULL AS lang_code, NULL AS course_type, NULL AS difficult, NULL AS "status", NULL AS create_date, NULL AS recency, NULL AS customer_name
WHERE
...
FROM
    {{ source('recommender_graph_db_dev', 'c3_dev_learning_course') }}
{% else %}
SELECT
    NULL AS item_id, NULL AS "name", NULL AS category_id, NULL AS lang_code, NULL AS course_type, NULL AS difficult, NULL AS "status", NULL AS create_date, NULL AS recency, NULL AS customer_name
WHERE
