...
FROM
    {{ source('recommender_graph_db_dev', 'c2_dev_core_group_members') }}
{% else %}
SELECT
    NULL AS group_id, NULL AS user_id, NULL AS customer_name
WHERE
...
FROM
    {{ source('recommender_graph_db_dev', 'c3_dev_core_group_members') }}
{% else %}
SELECT
    NULL AS group_id, NULL AS user_id, NULL AS customer_name
WHERE
