...
FROM
    {{ source('recommender_graph_db_dev', 'c2_dev_learning_course_rating_vote') }}
{% else %}
SELECT
    NULL AS user_id, NULL AS course_id, NULL AS rating, NULL AS customer_name
WHERE
...
FROM
    {{ source('recommender_graph_db_dev', 'c3_dev_learning_course_rating_vote') }}
{% else %}
SELECT
    NULL AS user_id, NULL AS course_id, NULL AS rating, NULL AS customer_name
WHERE
