{% set c1_table_exists = does_table_exists(
    ...
    'c1_dev_app7020_content_history_parquet'
) %}
{% set c2_table_exists = does_table_exists(
    'recommender_graph_db_dev',
    'c2_dev_app7020_content_history_parquet'
) %}
{% set c3_table_exists = does_table_exists(
    'recommender_graph_db_dev',
    'c3_dev_app7020_content_history_parquet'
) %}
...
FROM
    {{ source('recommender_graph_db_dev', 'c1_dev_app7020_content_history_parquet') }}
{% else %}
SELECT
...
FROM
    {{ source('recommender_graph_db_dev', 'c2_dev_app7020_content_history_parquet') }}
{% else %}
SELECT
    NULL AS user_id, NULL AS item_id, NULL AS "timestamp", NULL AS recency, NULL AS customer_name
WHERE
...
FROM
    {{ source('recommender_graph_db_dev', 'c3_dev_app7020_content_history_parquet') }}
{% else %}
SELECT
    NULL AS user_id, NULL AS item_id, NULL AS "timestamp", NULL AS recency, NULL AS customer_name
WHERE
